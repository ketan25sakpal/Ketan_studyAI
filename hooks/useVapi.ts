...
export function useVapi(book: IBook) {
    ...
    const [maxDuration, setMaxDuration] = useState(15 * 60); // Default 15 mins
    ...
    useEffect(() => {
        const handlers = {
                ...
                timerRef.current = setInterval(() => {
                    if (startTimeRef.current) {
                        ...
                        if (newDuration >= maxDurationRef.current) {
                            stop();
                            setLimitError('Session limit reached. Redirecting...');
                        }
                    }
                }, TIMER_INTERVAL_MS);
            },
            ...
            error: (error: any) => {
                // Handle cases where error might be an empty object or not a standard Error
                ...
                // Handle specific ejection/ended messages as normal terminations
                if (errorMessage.includes('ejection') || errorMessage.includes('meeting has ended')) {
                    // console.info('Vapi session ended normally:', error.message);
                    return; // The 'call-end' handler will deal with cleanup
                }
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            if (result.maxDurationMinutes) {
                setMaxDuration(result.maxDurationMinutes * 60);
            }
            ...
            const vapi = getVapi();
            if (!vapi) {
                setLimitError('Microphone not available. Please ensure you are in a supported browser.');
                setStatus('idle');
                return;
            }
    ...
    // Effect to auto-stop when duration reaches maxDuration
    const isActive =
        status === 'starting' ||
        status === 'listening' ||
        status === 'thinking' ||
    ...
    useEffect(() => {
        if (isActive && duration >= maxDuration) {
            stop();
            setLimitError('Session limit reached. Redirecting...');
        }
    }, [duration, maxDuration, isActive, stop]);
